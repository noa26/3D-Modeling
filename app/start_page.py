...
class StartPage(tk.Frame):
	...
	def capture_feature(self, frame):
		...
		capture_button = tk.Button(b_frame, **styles.BIG_BUTTON_STYLE, textvariable=capture_button_txt,
								command=lambda: capture(self.entries['file_type'], capture_button_txt))
		...
		m = tk.OptionMenu(frame, self.entries['file_type'], *options)
		m.config(**styles.MENU_STYLE, highlightthickness=0)
		m['menu'].config(**styles.MENU_STYLE)
		m.pack()
	...
	def update_display(ax):
		...
		mid_points = [((vertices[i][0] + vertices[i+1][0])/2, (vertices[i][1] + vertices[i+1][1])/2) for i in range(len(vertices)-1)]
